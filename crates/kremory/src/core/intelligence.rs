...
// TD-074 (2026-09-14): `ResolvedEntity` / `ProcessedFact` / `ProcessedIngestion` /
// `ProcessParams` / `IntelligencePipeline` (an unrealized orchestration layer
// predating `deferred_pipeline`) removed here — confirmed zero non-test callers
// crate-wide, no Story/ADR reference to a planned consumer. The two tests that
// exercised it (`test_pipeline_full_flow`, `test_pipeline_new_entity_not_resolved`)
// were removed with it; `MockResolver`/`MockContradictionDetector`/`MockExtractor`
// and their own direct tests below are UNCHANGED — they are not specific to the
