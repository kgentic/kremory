...
async fn reembed_all_overwrites_an_existing_entity_embedding() {
    ...
    assert_eq!(hits_before.len(), 1, "the one entity must be dense-searchable before re-embed");
    ...
    assert_eq!(stats.embedded, 1, "the one existing entity must be re-embedded");
