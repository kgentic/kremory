...
async fn spike_c6_async_gate_compiles_after_pub_crate_promotion() {
    ...
    //
    // This ACTUALLY walks up now. It previously seeded from `file!()` and popped
    // a hard-coded four levels, which silently encoded this file's depth in the
    // tree: moving the file changed what `file!()` returned, the same four pops
    // landed somewhere else, and `--manifest-path <that>/Cargo.toml` failed with
    // an error about kremory-eval that said nothing about the real cause. The
    // test-binary consolidation (tests/ -> tests/it/) added exactly one level and
    // broke it. Seeding from CARGO_MANIFEST_DIR (absolute, and independent of
    // where this source file lives -- the same primitive
    // `consistency_check_source_contains_pub_crate_promotions` below already
    // uses) plus a real search makes the depth irrelevant.
    let workspace_root = {
        ...
        loop {
            ...
            assert!(
                dir.pop(),
                "walked to the filesystem root from {} without finding a Cargo.toml \
