...
async fn spike_f2_same_name_two_namespaces() {
    ...
    let ctx_a = ns_a_view[0]
        .properties
        .get("context")
        .and_then(|v| v.as_str());
    let ctx_b = ns_b_view[0]
        .properties
        .get("context")
        .and_then(|v| v.as_str());
    assert_eq!(
        ctx_a,
        Some("first mention in ns-a"),
        "ns_a keeps its own properties"
    );
    assert_eq!(
        ctx_b,
        Some("second mention in ns-b"),
        "ns_b keeps its own properties"
    );
...
async fn spike_f2_second_insert_succeeds_per_namespace_open() {
    ...
    assert_eq!(
        ns_b.len(),
        1,
        "ns-b has its own independent row (per-namespace-open)"
    );
}
