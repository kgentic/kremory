...
//!     cargo test --features llm --test it e2e_local_llm:: -- --nocapture
...
three business days after the API merge to run the full regression suite.";
    ...
    async fn test_e2e_ingest_with_local_llm() {
        ...
        let exporter = crate::common::MetricsExporter::new(log_dir);
        exporter
    ...
    ///     cargo test --features llm --test it e2e_local_llm::test_e2e_nuextract -- --nocapture
    #[tokio::test(flavor = "multi_thread", worker_threads = 1)]
