...
// ─── A5: drift view — REMOVED (ADR-046 Amendment 2026-06-09, Option E) ──────
//
// Tests a5_drift_view_detects_type_mismatch, a5_drift_view_excludes_consumer_pinned,
// and a5_drift_view_excludes_cross_namespace have been removed.
//
// Rationale: v_entity_drift_candidates was a self-JOIN on entities looking for
// "same name, different entity_type_id" across rows. This is structurally impossible
// in kremory: id = normalize_name(text) means same entity name → same id → same row
// (UPSERT semantics). Two rows with matching names can't coexist. The self-JOIN
// condition `e2.id != e1.id AND TRIM(e2.id) = TRIM(e1.id)` can never be satisfied.
//
// The view was removed from Migration 012. Drift detection design is deferred to
// Phase E reclassify based on empirical precision measurement (Phase G TD-017 benchmark).
// See ADR-046 §Amendment-2026-06-09 (Option E) and TD-032.
