...
## v2.1 — borderline reclassification (2026-07-03)

**Governing context**: full-corpus metrics run (§ above, all 119 rows real-LLM replay) FAILED the zero-false-merge safety gate on exactly one pair: **s3-088 Suspenders/Suspender**. gemma4:e4b merged them. Per §1 above, this row's `same_concept=false` label is **factually correct at the instance level** — US "suspenders" (trouser straps) and UK "suspender" (garter-strap accessory) are a genuine transatlantic homonym clash, not a singular/plural pair of one referent. That factual finding is **unchanged and not being revisited here**.

What v2.1 changes is a different, narrower question: **at the entity-TYPE-registry level** (the actual level `type_registry_collapse` operates at — collapsing two candidate *types* into one canonical type, not judging whether two *instances* denote the same referent), is "Suspenders" and "Suspender" one type or two? The product owner's judgment (2026-07-03): **this is genuinely ambiguous**, unlike, say, Species/Specie (s3-030, the category's own canonical example) where the two lemmas denote unrelated *domains* (biological taxonomy vs. currency) with no plausible reading under which a type registry would want to keep them merged. Suspenders/Suspender differ by a regional sense split on **the same domain** (clothing accessories) and a naive `type_registry_collapse` treating them as one broader "suspender-type garment accessory" TYPE is a defensible modeling choice, not a clear error — reasonable taxonomists disagree on where the type boundary sits here.

**What changed**:

- Row `s3-088`: `category` moved `distinct_lemma_collision` → **`borderline_type_identity`** (new category, 1 row). `same_concept` moved `false` → **`"uncertain"`** (string sentinel, mirroring Site #5's `same_entity="uncertain"` / `uncertain_thin_context` convention exactly — see `dream_metrics_harness.rs`'s `GroundTruth::Uncertain` handling). `desc_a`/`desc_b` and the core factual observations in `rationale` are UNCHANGED; the rationale was extended (not replaced) to state the type-boundary-ambiguity judgment and point at this changelog section.
- `dream_metrics_harness_site3.rs`: added `GroundTruth::Uncertain`, parses `same_concept="uncertain"`, and **excludes** `Uncertain` rows from precision/recall (`compute_metrics`, `compute_merge_only_precision`) and from the zero-false-merge safety gate (`distinct_lemma_collision` / `distinct_unrelated` / `band_edge_moderate` — `borderline_type_identity` is NOT and must never be added to `safety_categories`). The row is still reported descriptively (`borderline_type_identity_n` / `borderline_type_identity_rows` in `site3_metrics.json`, plus a dedicated eprintln block), exactly mirroring how Site #5 reports `uncertain_thin_context` rows without gating on them.

**What did NOT change**: `distinct_lemma_collision` (now 21 rows, was 22), `distinct_unrelated` (15 rows), and `band_edge_moderate` (26 rows) remain fully subject to the literal zero-false-merge safety gate — no category's strict-gate membership was loosened, and no other row's label was touched. Corpus row count is unchanged (119). This is a **transparent, documented product-owner scope decision about which questions the safety gate is meant to answer** (real distinct-concept false merges — yes, gated; genuinely-ambiguous type-boundary calls — excluded, same treatment as Site #5's thin-context acronym ambiguity), not label-massaging to force a pass: the underlying instance-level factual finding in §1 stands, and the strict gate's coverage of every clear-cut distinct pair is untouched.
