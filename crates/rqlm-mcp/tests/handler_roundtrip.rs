//! Handler-level round-trip integration tests for rqlm-mcp.
//!
...
//!
//! Subprocess JSON-RPC plumbing is exercised by `cargo build -p rqlm-mcp`
//! producing a runnable binary + by rmcp's own integration suite. These
//! tests focus on the rqlm-mcp-specific surface: conversions + delegation
//! + error mapping.
...
use rql_core::provider::MockChatProvider;
use rql_memory::{
...
use rqlm_mcp::{
    ContextBlockParameters, IngestEpisodeParameters, RetrievedContextOutput, RqlmMcpServer,
...
/// returns deterministic canned results. Mirrors rqlm's internal stub
/// but in the rqlm-mcp test surface for explicit ownership.
#[derive(Default)]
...
fn bound_server() -> (RqlmMcpServer, Arc<StubGraphHandle>) {
    let graph = Arc::new(StubGraphHandle::default());
    let provider: Arc<dyn ChatProvider> = Arc::new(MockChatProvider::null());
    let server = RqlmMcpServer::new(graph.clone() as Arc<dyn GraphHandle>, provider);
    (server, graph)
}
...
async fn ingest_episode_bound_delegates_and_returns_structured_output() {
    ...
    let result = server
        .rqlm_ingest_episode(Parameters(sample_ingest_params()))
        .await
    ...
    // Verify rqlm forwarded the params to the graph handle.
...
async fn run_dream_phase_bound_delegates_and_returns_counts() {
    ...
    let result = server
        .rqlm_run_dream_phase(Parameters(params))
        .await
...
async fn context_block_renders_entities_template_without_graph() {
    ...
    let result = server
        .rqlm_context_block(Parameters(params))
        .await
...
async fn ingest_episode_unbound_returns_graph_not_bound_error() {
    ...
    assert!(
        err.message.contains("rqlm_ingest_episode"),
        "error must carry tool name: {}",
        err.message
    );
    assert!(
        err.message.contains("RqlmMcpServer::new"),
...
async fn search_unbound_returns_graph_not_bound_error() {
    ...
    assert_eq!(err.code, ErrorCode::INTERNAL_ERROR);
    assert!(err.message.contains("rqlm_search"));
}
...
async fn run_dream_phase_unbound_returns_graph_not_bound_error() {
    ...
    assert_eq!(err.code, ErrorCode::INTERNAL_ERROR);
    assert!(err.message.contains("rqlm_run_dream_phase"));
}
...
async fn ingest_episode_rejects_bad_source_kind_with_invalid_params() {
    ...
    let err = server
        .rqlm_ingest_episode(Parameters(params))
        .await
