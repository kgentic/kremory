...
//! stream.
//!
//! This test spawns the actual built `rqlm-mcp-server` binary, speaks
...
//! 4. **`tools/call rqlm_context_block`** returns a structured payload on
//!    the unbound server (this tool is pure — no graph required).
//! 5. **`tools/call rqlm_search` on unbound server returns an error** with
...
/// Spawn the real binary. `env!("CARGO_BIN_EXE_rqlm-mcp-server")` is
...
fn spawn_server() -> (Child, ChildStdin, BufReader<ChildStdout>) {
    let path = env!("CARGO_BIN_EXE_rqlm-mcp-server");
    let mut child = Command::new(path)
        ...
        .env("RUST_LOG", "warn")
        .spawn()
        .expect("rqlm-mcp-server binary must spawn");
...
async fn initialize(stdin: &mut ChildStdin, stdout: &mut BufReader<ChildStdout>) {
    let init = json!({
        ...
        "params": {
            ...
            "capabilities": {},
            "clientInfo": { "name": "rqlm-mcp-e2e-test", "version": "0.0.1" }
        }
    });
...
#[tokio::test]
async fn tools_list_advertises_all_four_rqlm_tools_under_canonical_names() {
    let (child, mut stdin, mut stdout) = spawn_server();
    initialize(&mut stdin, &mut stdout).await;
    ...
    assert_eq!(
        ...
        "all four canonical rqlm tools must be advertised; got: {names:?}"
    );
...
#[tokio::test]
async fn tools_call_rqlm_context_block_works_on_unbound_server() {
    ...
    let structured = result
        ...
        .expect("rqlm_context_block returns structured content");
...
#[tokio::test]
async fn tools_call_rqlm_search_on_unbound_server_returns_graph_not_bound_error() {
    // search requires a bound graph. The default `rqlm-mcp-server` binary
    // starts unbound (no `RqlmMcpServer::new`), so this call MUST return
    ...
    assert!(
        message.contains("rqlm_search") || message.contains("RqlmMcpServer::new"),
