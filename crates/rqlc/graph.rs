...
use crate::schema::{Entity, EpisodicEdge, Fact, TemporalGraph};
...
impl TemporalGraph {
    ...
    pub async fn insert_fact(
        ...
        let row = rows.next().await?.ok_or_else(|| {
            crate::error::RqlError::Other(anyhow::anyhow!("no rowid after insert_fact"))
        })?;
    ...
    pub async fn insert_episode(
        ...
        let mut rows = self.conn.query("SELECT last_insert_rowid()", ()).await?;
        let row = rows.next().await?.ok_or_else(|| {
            crate::error::RqlError::Other(anyhow::anyhow!("no rowid after insert_episode"))
        })?;
    ...
    pub async fn insert_fact_with_group(
        ...
        let mut rows = self.conn.query("SELECT last_insert_rowid()", ()).await?;
        let row = rows.next().await?.ok_or_else(|| {
            crate::error::RqlError::Other(anyhow::anyhow!("no rowid after insert_fact_with_group"))
        })?;
        let fact_id = row.get::<i64>(0)?;
        if let Some(ov) = object_value {
            self.conn
    ...
    pub async fn insert_episode_with_group(
        ...
        let mut rows = self.conn.query("SELECT last_insert_rowid()", ()).await?;
        let row = rows.next().await?.ok_or_else(|| {
            crate::error::RqlError::Other(anyhow::anyhow!(
