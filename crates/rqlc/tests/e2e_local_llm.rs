...
mod llm_tests {
    ...
    use rql_core::config::PipelineConfig;
    use rql_core::extraction::NuExtractExtractor;
    use rql_core::ingest::RqlGraph;
    use rql_core::provider::{ChatProvider as _, NullEmbeddingProvider};
    use super::common::build_llm;
    use rql_core::schema::TemporalGraph;
...
three business days after the API merge to run the full regression suite.";
    ...
    async fn test_unconstrained_entity_extraction() {
        use rql_core::provider::{chat_msg_system, chat_msg_user};
        ...
        let result: Result<Box<dyn rql_core::provider::ChatResponse>, _> =
