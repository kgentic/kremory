...
///
/// Run without LLM (CI):
///   cargo test -p rql-core --test spike_programmatic_first -- --nocapture
///
/// Run with real LLM (benchmark):
///   RQL_QWEN3B_MODEL_PATH=/path/to/qwen2.5-3b-instruct-q4_k_m.gguf \
///     cargo test --features llm -p rql-core --test spike_programmatic_first -- --nocapture
...
mod spike {
    ...
    use rql_core::extraction::ProgrammaticFirstExtractor;
    use rql_core::intelligence::{EntityExtractor, ExtractionContext};
    use rql_core::provider::MockChatProvider;
    use rql_core::text_utils::{compute_pmi_bigrams, OovAuditor};
...
mod spike_real_llm {
    ...
    use rql_core::provider::{chat_msg_system, chat_msg_user, ChatProvider as _};
