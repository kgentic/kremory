...
use crate::config::{ContentType, PipelineConfig};
use crate::contradiction::TwoPoolDetector;
use crate::error::Result;
use crate::extraction::NuExtractExtractor;
...
use crate::resolver::{normalize_name, CascadeResolver, UnionFind};
...
impl<L: ChatProvider, Emb: EmbeddingProvider> RqlGraph<L, Emb> {
    ...
    pub async fn ingest(
        ...
        #[cfg(feature = "ner")]
        {
            ...
            static GLINER: OnceLock<crate::ner::GlinerExtractor> = OnceLock::new();
            if GLINER.get().is_none() {
                let g = crate::ner::GlinerExtractor::new().map_err(crate::error::RqlError::from)?;
                let _ = GLINER.set(g);
            }
    ...
    pub async fn ingest_with<E: EntityExtractor>(
        ...
        for fact in &all_facts {
            ...
            let pool_b: Vec<crate::schema::Fact> =
...
mod tests {
    use super::*;
    use crate::intelligence::{ExtractionContext, ExtractionResult};
    use crate::provider::{MockChatProvider, MockEmbeddingProvider};
    use std::collections::HashMap;
    ...
    async fn test_ingest_creates_entities_and_facts() {
        let rql = make_rql_with_mock().await;
        let extractor = crate::extraction::NuExtractExtractor::new(Arc::clone(&rql.llm));
        let result = rql
