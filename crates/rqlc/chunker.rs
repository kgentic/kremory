use crate::config::{ChunkConfig, ContentType};
...
impl Chunker {
    ...
    /// Split text into chunks based on content type and density.
...
mod tests {
    ...
    fn config_with_overlap(min: usize, max: usize, density: f64, overlap: usize) -> ChunkConfig {
        ChunkConfig {
            min_tokens: min,
            max_tokens: max,
    ...
    fn test_short_text_single_chunk() {
        ...
        let chunks = chunker.split(text, &ContentType::Text);
        assert_eq!(
            chunks.len(),
            1,
    ...
    fn test_json_passthrough() {
        ...
        let chunks = chunker.split(json, &ContentType::Json);
    ...
    fn test_message_splits_on_paragraphs() {
        ...
        let text = format!("{}\n\n{}", para1, para2);
        let chunks = chunker.split(&text, &ContentType::Message);
        assert!(
            chunks.len() >= 2,
    ...
    fn test_zero_overlap_is_noop() {
        ...
        let chunker_a = Chunker::new(cfg_no_overlap);
        let chunker_b = Chunker::new(cfg_with_overlap_val);
        let words: Vec<String> = (0..250).map(|i| format!("word{:03}", i)).collect();
        let text = words.join(" ");
        let chunks_a = chunker_a.split(&text, &ContentType::Text);
        let chunks_b = chunker_b.split(&text, &ContentType::Text);
        assert_eq!(
            chunks_a, chunks_b,
