...
//
// Phase G / TD-003: verifies that JsEpisode returned by getBySourceId
...
test('getBySourceId: JsEpisode.sourceId matches ingest slug', async () => {
  ...
  try {
    ...
    const episodes = await mem.getBySourceId(slug, { namespace: 'parity-test' });
    assert.ok(Array.isArray(episodes), 'getBySourceId must return an array');
    assert.ok(episodes.length > 0, 'getBySourceId must return at least one episode');
...
test('getBySourceId: JsEpisode.contentHash is 64-hex SHA-256', async () => {
...
test('getBySourceId: JsEpisode has all expected fields', async () => {
  ...
  try {
    ...
    const requiredFields = ['id', 'sourceId', 'sourceUri', 'content', 'timestamp', 'contentHash'];
