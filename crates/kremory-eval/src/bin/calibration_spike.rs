...
async fn run_spike(
    ...
    Ok(RunResult {
        ...
        // Record the model FILE NAME only, never the resolved absolute path. This
        // struct is serialised straight into `baselines/v0.1.4-judge-calibration.json`,
        // which is COMMITTED — so an absolute path here writes the operator's home
        // directory into the repository on every run. Enforced at the emit site rather
        // than scrubbed afterwards, because the file is regenerated by this binary and
        // any scrub would be undone by the next run. The baseline compares judge
        // agreement across model identities; the containing directory is not part of
        // that identity.
