...
| fact | value |
|---|---|
| tree version | ~~`Cargo.toml` says **0.6.0**~~ — **stale, `Cargo.toml` now says 0.7.0 (bumped 2026-09-02).** 0.6.0 was the last version live on crates.io as of session start (below). Many commits touch `crates/` since that publish, incl. TD-206/TD-203/TD-231/TD-232 fixes and the ADR-068 as_of-for-content-search extension. **`kremory-napi` is NOT the same gate** — corrected 2026-09-02: it has never been published to crates.io (`curl .../crates/kremory-napi` → `"does not exist"`), and now never will be — `publish = false` set in its `Cargo.toml` deliberately (2026-09-02), because it's a workspace-internal napi-rs build artifact with zero real Rust consumers; its actual (unstarted) distribution surface is npm, `@kgentic/kremory-node`, a wholly separate registry/publish step. Its version field still tracks alongside `kremory`'s for internal consistency, but that number is not a publish-readiness signal for it. |
