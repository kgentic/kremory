...
| fact | value |
|---|---|
| tree version | **0.9.0 — bumped 2026-09-10, PREPARED NOT PUBLISHED (crates.io still holds 0.8.0). BREAKING: `forget().execute()` returns `ForgetOutcome`, not `u64`, so this is 0.9.0 and not the 0.8.1 patch that was planned. `cargo publish --dry-run` exit 0. Publishing remains a standing HITL gate.** Historical below:  ~~`Cargo.toml` says **0.6.0**~~ — **stale, `Cargo.toml` now says 0.7.0 (bumped 2026-09-02).** 0.6.0 was the last version live on crates.io as of session start (below). Many commits touch `crates/` since that publish, incl. TD-206/TD-203/TD-231/TD-232 fixes and the ADR-068 as_of-for-content-search extension. **`kremory-napi` is NOT the same gate** — corrected 2026-09-02: it has never been published to crates.io (`curl .../crates/kremory-napi` → `"does not exist"`), and now never will be — `publish = false` set in its `Cargo.toml` deliberately (2026-09-02), because it's a workspace-internal napi-rs build artifact with zero real Rust consumers; its actual (unstarted) distribution surface is npm, `@kgentic-ai/kremory-node`, a wholly separate registry/publish step. ~~Its version field still tracks alongside `kremory`'s for internal consistency~~ **— WRONG, corrected 2026-09-05: it does not. `kremory-napi` is at `0.4.1` (Cargo.toml AND package.json agree) while `kremory` is at `0.7.1`. The two have not tracked each other for three minor versions.** That number is not a publish-readiness signal for it either way. ~~Tree version is now `0.7.1` (bumped 2026-09-05), PREPARED BUT NOT PUBLISHED — crates.io still holds 0.7.0.~~ **Bumped again to `0.8.0` on 2026-09-08: two BREAKING API changes landed on top of the unpublished 0.7.1 prep (`DreamRequest` now requires `.execute()`; `NoEmb`/`WithEmb` renamed to `NoEmbedder`/`WithEmbedder` — see `crates/kremory/CHANGELOG.md` `[0.8.0]`). Per semver for a pre-1.0 crate, a breaking change bumps the minor (0.7.x → 0.8.0), not the patch. Nothing between 0.7.0 and 0.8.0 was ever published, so 0.7.1 never existed as a live version — 0.8.0 absorbs everything prepared since 0.7.0. `cargo publish` remains a standing HITL gate, not exercised here.** |
| **crates.io** | ✅ **0.8.0 LIVE, published 2026-09-08, `default = ["content-search"]`, not yanked** — verified against the sparse index after upload, not from the publish log. `cargo add kremory` now gets the 0.8.0 breaking API (`DreamRequest::execute()`, `NoEmbedder`/`WithEmbedder`). ⚠️ **A first publish attempt that day FAILED with `403 Forbidden: authentication failed`, and my recorded diagnosis was WRONG.** I wrote "the token in `~/.cargo/credentials.toml` is expired/revoked/wrong-scope". The real cause: **`credentials.toml` holds a STALE token, while the working one lives in `.env` as `CRATES_IO_API_KEY`** — both 35 chars, so token length distinguished nothing and I asserted a cause I had not tested. Publish with `cargo publish -p kremory --token "$CRATES_IO_API_KEY"`, or refresh `credentials.toml`. Historical: ⚠️ **0.7.0 was newest live (verified via sparse index 2026-09-08) — the 0.6.0 claim below is STALE.** **0.8.0 publish ATTEMPTED 2026-09-08 and BLOCKED: `403 Forbidden: authentication failed` at the upload step. NOTHING was published — index confirms `0.8.0 present: False`, no partial state. Packaging is fine (`--dry-run` exit 0, 154 files). `~/.cargo/credentials.toml` holds a 35-char token crates.io rejects → expired/revoked/wrong scope. FOUNDER ACTION: mint a `publish-update` token at crates.io/settings/tokens, `cargo login`, retry. Plan + full detail: `.ai-docs/plans/ship-rust-then-node-parity-2026-09-08.md`.** Historical: ✅ **0.6.0, live since 2026-08-11T11:59:52Z, `default = ["content-search"]`.** `cargo add kremory` gets the CORRECT default. ~~**0.5.0**, `default = []` — every user gets search OFF (~25.7% vs ~86.2%)~~ **← WRONG-VERIFIED, corrected 2026-08-12.** That claim was false for >24h and was inherited by the 2026-08-12 session note and the consolidation plan, whose flagship "Phase 0 — publish the fix" proposed re-doing work already done. Verified two independent ways: `curl https://index.crates.io/kr/em/kremory` (sparse index shows `"default":["content-search"]` on 0.6.0) and `curl https://crates.io/api/v1/crates/kremory` (`max_version: 0.6.0`). **Re-check the registry, not the doc, before ever asserting what users receive.** |
| `default` features | `["content-search"]` in **all three** crates (`kremory`, `kremory-mcp`, `kremory-napi`) — verified 2026-08-06 |
| workspace nextest | **STATE THE SCOPE — and `-p kremory -p kremory-mcp` IS NOT ENOUGH.** ⚠️ **Measured 2026-09-21: `--workspace` runs 2098 tests; `-p kremory -p kremory-mcp` runs 1910. That 188-test gap contains the napi PARITY GATE**, and a whole session ran the narrow scope eight times while the gate sat red on a real API drift nobody could see. Line 87 of this very file already warned that the parity test is invisible to a `-p kremory` run. **Use `--workspace` for any gate you intend to trust.** Two different numbers, both correct — but only one of them covers the repo. `-p kremory -p kremory-mcp` (`content-search,test-utils`) → ~~1699~~ ~~1742~~ ~~1748~~ ~~1790 passed / 1790 run, 6 skipped, re-measured 2026-09-06~~ ~~1800 passed / 1800 run, 6 skipped, re-measured 2026-09-08~~ **1910 passed / 1910 run, 7 skipped, end of 2026-09-18 session** (`cargo nextest run -p kremory -p kremory-mcp --features content-search,test-utils`, 363s, exit 0). Was **1908 / 6 skipped** at the start of that session; the +2 run and +1 skipped are exactly the two `cross_process_one_database` tests plus the ignored child entrypoint they spawn. ⚠️ **BOTH stored numbers were stale and they disagreed with each other** — this row said 1800 and the TD register's TD-254 entry said 1891 as of 2026-09-15. Neither was measured; the real figure is 1908. That is the whole reason the 2026-09-18 plan makes re-measurement its phase 0. Same run: clippy `--workspace --all-targets --all-features` exit 0, **0 warnings**; doctests `-p kremory` **27 passed / 5 ignored** (the "26" below is also stale — re-measured 2026-09-18). Historical: at the SAME scope 1790 → 1800, so the +10 was real test growth from the 2026-09-07/08 doc-and-guard commits, not a scope difference; `cargo build --workspace --all-features` exit 0. Prior figure at scope `-p kremory -p kremory-mcp --features content-search,test-utils`. Grew 1751 -> 1772 by exactly the 21 tests added that day: 11 for ADR-080 prior-turn replay (7 graph-layer + 4 prompt-render), 6 for the `update_episode_metadata` multi-row clobber fix, 4 for the dream-reclassify registry bounds check. Every one was verified RED before being trusted — by neutering the mechanism it guards and confirming the failure, not by assumption. Then +13 in the same day's debt-clearance pass: 10 for TD-235 namespace-scoping the episode metadata/uri writers, 3 for the ADR-reference-integrity guard and the consistency-check registry bounds check. Prior figures this session: ~~1772~~ ~~1751 passed / 1751 run, 6 skipped, 2026-09-05~~ (+3 = the `public_surface_hygiene` guard, its own sensitivity test, and an exemption-liveness test; both verified RED-on-injected-leak before being trusted). Prior figure: (grew +49 across this session: TD-231/TD-232 tests, 10 ADR-068 as_of-for-content-search tests, 6 TD-232/TD-234 `split_for_embedding` tests — 4 direct unit tests on the splitter itself + 2 real end-to-end tests in `tests/it/td234_split_for_embedding_closes_the_loop.rs` reproducing TD-232's own oversized-document fixture; all new tests this session verified sensitive via neuter-and-confirm-RED before being trusted). Workspace-wide → **1782/1782, 6 skipped** (2026-08-06, not re-measured at workspace scope this session — the `-p kremory -p kremory-mcp` number above is the one actually re-verified). The 2026-08-12 run grew **1682 → 1699** by exactly the 17 tests added that day: 4 lexical date-merge, 1 zero-padding regression, 4 write-gate temporal veto, 8 NULL-embedding gap-fill. ⚠️ An earlier session read the 1682-vs-1782 gap as a REGRESSION and nearly halted on it; it is a scope difference. Never write a test count here without its package scope AND feature flags. |
| doctests | `-p kremory` → ~~25~~ ~~26~~ ~~27~~ ~~26 passed, 5 ignored, re-verified 2026-09-04~~ **27 passed, 5 ignored, re-measured 2026-09-18** (`cargo test -p kremory --doc`, exit 0). ⚠️ This row has now oscillated 26↔27 three times, each "re-verified". The oscillation is real test churn, not a bad instrument — but it means **the number here is only ever a snapshot; re-run the one command before quoting it**. Historical: 26 at 2026-09-04 (pre-release gate for 0.7.0, when the prior "27" from 2026-09-02 did not reproduce). `--workspace` → **25 passed, 6 ignored** (2026-08-06, not re-measured workspace-wide this session). ⚠️ nextest does NOT run these — separate tier. **State the scope or the number is meaningless**: the 6th ignored is `kremory-eval/src/layer_b/graph_integrity.rs:19`; `kremory-mcp`/`kremory-napi` have 0 and `kremory-admin` has no lib target at all. A prior "CLAUDE.md says 5, measured 6" drift note was NOT a drift — both numbers were right at different scopes. |
...
the guards more than scripts nobody remembers. **It exits 0 as of 2026-09-21 — all nine guards PASS, zero FAIL, zero SKIP.** TD-243 is CLOSED. Seven oversized files were genuinely SPLIT (kremory-http 3147, discover_types 2911, graph/tests 2072, napi convert 1987, napi lib 1626, core/config 1631, supersession/reverse), and the remaining 17 entries were re-baselined ON PURPOSE after a per-file audit found NO avoidable bloat in any of them — every line traced to a named TD, a real incident, or a doc-correctness fix. The 2 PINNED entries were never raised. ⚠️ **The prohibition on `--update` still stands for UNEXAMINED growth** — that is what it was written against. A re-baseline is legitimate only with a written per-file justification, which is what the two audits in `.ai-docs` provide. ⚠️ **The tool has THREE tiers**: PINNED (`pin=<TD>`), WATCHED (baseline >= 1500), and `NEW_FILE_CEILING=800` — *a NEW file may not be born a god-file*. That third tier is INVISIBLE until you commit, because the script enumerates via `git ls-files` and an untracked new file does not exist to it; two splits that day each produced an over-ceiling test file and had to be re-split. Compute `wc -l` yourself before `git add`. The eval floor SKIPS unless you pass `--results`, because producing a
...
- **The pipe hazard is REAL here too:** `false | tail -1; echo $?` → **0**. ⚠️ **And `${PIPESTATUS[0]}` does NOT rescue you — that is a BASH array; zsh spells it `$pipestatus` (lowercase). Using the bash name silently falls back to `$?`, i.e. the LAST command in the pipe.** This fired on 2026-09-08: `cargo publish … | tail; echo "EXIT=${PIPESTATUS[0]:-$?}"` printed **EXIT=0 for a publish that failed with 403**. Keep capturing exit
...
**Live plan:** `.ai-docs/plans/ship-rust-then-node-parity-2026-09-08.md` — ship the Rust crate first (BLOCKED on a crates.io token), then Node parity as a stoppable workstream. Supersedes, but does not delete, `.ai-docs/plans/v1-debt-and-gap-closure-2026-08-06.md`. Ordering note: the
...
5. **EVAL FLOOR (the eval harness's per-question pass rate — NOT retrieval recall@k) — `bash scripts/check-eval-floor.sh results/floor/<run>.json` — MANDATORY
   ...
   **The refresh is not optional bookkeeping — it is the half that keeps the gate real.** The
   ...
   and commit it in the same change** — `bash scripts/check-eval-floor.sh --update results/floor/<run>.json`.
   ...
   **The floor is POPULATED as of 2026-09-21: 89.29% over 1,540 scored questions**, re-baselined from `results/floor/eval-floor-20260918.slim.json` — the first run whose GRAPH INTEGRITY was actually verified. The prior floor was derived from a run recording `graph_integrity: "skipped"`, i.e. a baseline built on an unverified graph, which is the GraphQLite defect this gate was written against. ⚠️ Floor provenance records rrf_k, the weights, features and git_sha but **NOTHING about the EMBEDDING MODEL** — an embedder swap moves recall more than any weight the drift check watches and is invisible to it; `eval_floor.py` now warns. Historical: ~~89.16% as of 2026-09-08~~ (multi-hop 63.54 ·
   open-domain 95.6 · single-hop 80.85 · temporal 87.23), from `results/floor/eval-floor-20260908.json`
   at `db29b3bd` with `rrf_k=1`, `content-search`, `server_mode=hybrid`, substring scorer. The source
   run is COMMITTED — a floor citing a file nobody has is exactly GraphQLite's defect, where
   `generated_from` points into a gitignored directory.

   ⚠️ The substring scorer is **order-invariant** — it sees set-membership changes (the class ADR-078
   was) but is BLIND to pure reordering. A reranking regression will pass this gate. Use the paid
   grader for anything ordering-sensitive.
