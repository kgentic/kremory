...
the guards more than scripts nobody remembers. **It exits 1 today** — `check-file-size-ratchet.sh` is red on **18 entries** (5 WATCHED, 13 over +25), re-measured 2026-09-21, ~~100 files~~. Tracked as **TD-243**, and it must NOT be silenced with `--update`. ⚠️ **The tool has THREE tiers, not two**: PINNED (`pin=<TD>`, fails on any growth), WATCHED (baseline >= 1500, fails on any growth), and `NEW_FILE_CEILING=800` — *a NEW file may not be born a god-file*. That third tier is invisible until you COMMIT, because the script enumerates via `git ls-files` and an untracked new file does not exist to it. Two splits on 2026-09-21 each produced an over-ceiling test file and had to be re-split; compute `wc -l` yourself before `git add`. The eval floor SKIPS unless you pass `--results`, because producing a
