...
- **3 standing test fails on the recall branch (TD-138, 2026-07-23):** `facade_as_of_warn::as_of_inside_window_includes_fact` + `::as_of_none_is_unaffected_regression_pin` + `with_facts_integration::td116_recall_returns_connected_facts_under_null_embedder` — a silent recall regression (all 3 were GREEN when TD-067/TD-116 closed mid-July); NOT caused by the dense-arm commit (proven on clean HEAD). Bisect + cause-fix tracked as TD-138. (TD-013 `with_facts_empty_vec_equivalent_to_no_facts` was verified **CLOSED 2026-07-15**, `1 passed, 0 failed`.) The old "napi parity-cap fail" was never real (the "103>100" number was fabricated per `feedback_subagent_fabricates_gate_results`). NB: this "Active Sprint" block still describes the 0.3.2 line and is broadly stale — current baseline is **0.5.0** (crates.io); see `.ai-docs/planning/pre-public-launch-readiness-roadmap-2026-07-15.md` + the MVP-to-public-flip execution plan for live state.
