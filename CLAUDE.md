...
| fact | value |
|---|---|
| tree version | ~~`Cargo.toml` says **0.6.0**~~ — **stale, `Cargo.toml` now says 0.7.0 (bumped 2026-09-02, still NOT published — `cargo publish` remains a standing HITL gate, untouched this session).** 0.6.0 IS live on crates.io (below); this version number is spent. `kremory-napi` is at 0.4.1 (same reason). Many commits touch `crates/` since the 0.6.0 publish, incl. TD-206/TD-203/TD-231/TD-232 fixes and the ADR-068 as_of-for-content-search extension — **the tree is ahead of the registry, and cannot be published as-is without a deliberate go-ahead.** |
