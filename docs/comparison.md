...
> Data current as of 2026-05-22. All competitor cells verified against primary sources (GitHub repos, crates.io, npm, PyPI, official docs). kremory cells reflect this project's locked architectural decisions (licensing decision plus five engine-architecture decisions) and its architecture spec. Sources cited per cell.
>
...
|---|---|---|---|---|---|---|
| **Language** | Pure Rust | Pure Rust (93% bytes) [codemem section 1] | Pure C (95% bytes) [store.c verified] | Python | Python | Python |
| **License** | Apache-2.0 [ADR-v3] | Apache-2.0 [codemem Cargo.toml] | MIT [LICENSE: "Copyright (c) 2025 DeusData"] | Apache-2.0 [licensing-revisit section 3] | Apache-2.0 [licensing-revisit section 3] | Apache-2.0 (Graphiti) [licensing-revisit section 3] |
...
| **Storage substrate** | libSQL (Turso-compatible; embedded or remote) | rusqlite 0.39 bundled SQLite — local only [codemem Cargo.toml section 3.2] | Pure C + SQLite (`cbm_store_t` opaque handle) [src/store/store.c verified] | Postgres / hosted cloud [licensing-revisit section 3] | SQLite + cloud [licensing-revisit section 3] | Postgres + Neo4j / hosted cloud [licensing-revisit section 3] |
| **Graph implementation** | Home-grown bi-temporal graph on libSQL | petgraph 0.8 persisted to SQLite [codemem section 3.3] | SQLite tables only — no graph library; semantic graph represented as edge rows [store.c] | Python networkx-style / cloud graph [licensing-revisit section 3] | Python native / cloud [licensing-revisit] | Custom Python graph engine [licensing-revisit section 3] |
| **Embedding model** | BYOM — `Arc<dyn EmbeddingProvider>` required; no model bundled ([ADR-002](adr/adr-002-byom-distribution-moat.md)) | Bundles Candle + BAAI/bge-base-en-v1.5 (~440MB) [codemem Cargo.toml: candle-core, hf-hub] | Bundled embedding via tree-sitter semantic tokens — not a neural embedder [README] | OpenAI embeddings + Qdrant / cloud [licensing-revisit] | OpenAI / configurable [licensing-revisit] | OpenAI / configurable [licensing-revisit] |
| **crates.io publishable** | Yes — engine crate is ~1MB (BYOM, no model) ([ADR-002](adr/adr-002-byom-distribution-moat.md)) | Yes — `codemem` publishes on crates.io | N/A (not a Rust crate) | N/A (Python) | N/A (Python) | N/A (Python) |
| **Time model** | Two-axis: `recorded_at` (TX time, immutable) + `valid_from`/`valid_to` (valid time, mutable) ([ADR-003](adr/adr-003-bitemporal-audit-compliance.md)) | One axis: `valid_from`/`valid_to` on nodes/edges via migrations 003+015; no `recorded_at` [codemem section 3.6] | None — no temporal model in storage layer [README, store.c verified] | None — vector recency only [licensing-revisit section 3] | None [licensing-revisit section 3] | Partial — entity/fact timestamps, one clock [licensing-revisit section 3] |
| **Contradiction resolution** | Active engine: `ContradictionResolution` enum (Superseded / Merged / Forked / Ignored); `valid_to` set on invalidated facts ([ADR-003](adr/adr-003-bitemporal-audit-compliance.md)) | Labels only: `Contradicts`/`InvalidatedBy`/`Supersedes` edge types in `RelationshipType` enum; no resolver algorithm found in source [codemem section 3.7] | None [README, store.c] | None [licensing-revisit section 3] | None [licensing-revisit section 3] | Partial: supersession edges; no active resolver [licensing-revisit section 3] |
| **MCP tool count** | ✏️ 5 tools (remember / recall / dream / list_mutations / undo) — shipped in this repo (`crates/kremory-mcp`), not yet published as an installable package [see README "Node.js / MCP"] | 32 tools via JSON-RPC stdio or HTTP [codemem section 3.9, unverified since 2026-05-22] | 14 tools [README: "14 MCP tools", unverified since 2026-05-22] | Yes — Python SDK + cloud API | Yes — Python SDK + REST | Yes — Graphiti Python SDK |
...
| **Embedded (no server process)** | Yes — libSQL file, no server process required ([ADR-002](adr/adr-002-byom-distribution-moat.md)) | Yes — rusqlite file, no server process | Yes — pure C binary, no server | No — requires Postgres/cloud | No — requires external services | No — requires Postgres/Neo4j |
...
| **Active development** | ✏️ Active — 0.7.0 released 2026-09-04, LoCoMo benchmark added (see README) | Active — v0.18.0, weekly releases [codemem section 4, unverified since 2026-05-22] | Active [README arXiv 2603.27277, unverified since 2026-05-22] | Active, Series A funded | Active, seed funded | Active, YC funded |
