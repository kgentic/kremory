...
| **Embedded (no server process)** | Yes — libSQL file, no server process required | Yes — rusqlite file, no server process | Yes — pure C binary, no server | No — requires Postgres/cloud | No — requires external services | No — requires Postgres/Neo4j |
